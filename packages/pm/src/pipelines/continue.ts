...
import { prepareContinueSkillContext } from "../skills/index.js";
...
async function runContinueInner(description: string, maxIterations: number, pctx: PipelineContext) {
  const prepared = await prepareContinueSkillContext(description, pctx);
  pctx = prepared.pctx;
  const { ctx } = pctx;
  ...
  await pctx.runAgentFn(
    "gh-issue-creator",
    "Decompose PRD.md into vertical-slice GitHub issues. Focus on the ## Next section — the ## Done section is context only. If `.forgeflow/BOOTSTRAP.md` exists, read it and treat its locked inputs as binding for setup-sensitive slices: preserve exact starter/template identifiers, package manager choices, scaffold commands, versioned tooling choices, and explicit use/avoid constraints where relevant. Read the issue-template skill for the standard format. In greenfield or nearly empty repos, create exactly one explicit initial scaffold/bootstrap issue first, make it the only dependency-free issue, and then create feature slices that depend directly or transitively on it. That scaffold issue must still establish a real feature or domain boundary rather than treating generic roots such as src/, app/, server/, client/, test/, or tests/ as owning boundaries.",
    agentOpts,
  );
