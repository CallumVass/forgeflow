...
// `exports` map and NOT re-exported from `pipeline.ts` on purpose: run
// directory lifecycle lives at a single boundary (see `withRunLifecycle`
// in `context.ts`, wired into each extension's `execute` callbacks in
// `packages/dev/src/index.ts` / `packages/pm/src/index.ts`). Pipelines
// must NOT create or archive run directories themselves — they receive
// an opaque `allocSessionPath` via `RunAgentOpts` and otherwise never
// touch the on-disk layout. Keeping this module physically unexported
// makes cross-package import a hard Node module resolution error, so
// the rule is enforced without a grep script.
//
 ...
 */
type ArchiveOutcome = "success" | "failed" | "cancelled";

/**
 * Handle returned by `createRunDir`; closed over by the session-path
 * allocator. Kept non-exported for the same reason as `ArchiveOutcome`:
 * callers currently receive it only via structural inference on
 * `createRunDir`'s return type.
 */
