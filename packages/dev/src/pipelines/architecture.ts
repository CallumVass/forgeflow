...
import { IMPLEMENT_ALL_LABELS } from "./implement-all.js";

/**
 * Label applied to RFC issues generated by the architecture pipeline. Sourced
 * from `IMPLEMENT_ALL_LABELS` so the architecture producer and the
 * `/implement-all` consumer stay in lock-step.
 */
const ARCHITECTURE_ISSUE_LABEL = IMPLEMENT_ALL_LABELS[1];
...
export async function runArchitecture(pctx: PipelineContext, opts?: { runAgentFn?: RunAgentFn }) {
  ...
  for (const [i, candidate] of selectedCandidates.entries()) {
    ...
    const rfcResult = await runAgentFn(
      "architecture-reviewer",
      `Based on the following architectural analysis, generate a detailed RFC and create a GitHub issue (with label "${ARCHITECTURE_ISSUE_LABEL}") for this specific candidate.\n\nCANDIDATE:\n${candidate.body}\n\nFULL ANALYSIS (for context):\n${candidateContext}`,
